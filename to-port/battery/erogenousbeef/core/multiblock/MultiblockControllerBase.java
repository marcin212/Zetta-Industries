...
public abstract class MultiblockControllerBase {
	...
	public void attachBlock(IMultiblockPart part) {
		...
		if(minimumCoord != null) {
			if(part.getPos().getX() < minimumCoord.x) { minimumCoord.x = part.getPos().getX(); }
			if(part.getPos().getY() < minimumCoord.y) { minimumCoord.y = part.getPos().getY(); }
			if(part.getPos().getZ() < minimumCoord.z) { minimumCoord.z = part.getPos().getZ(); }
		}
		
		if(maximumCoord != null) {
			if(part.getPos().getX() > maximumCoord.x) { maximumCoord.x = part.getPos().getX(); }
			if(part.getPos().getY() > maximumCoord.y) { maximumCoord.y = part.getPos().getY(); }
			if(part.getPos().getZ() > maximumCoord.z) { maximumCoord.z = part.getPos().getZ(); }
		}
	...
	public void detachBlock(IMultiblockPart part, boolean chunkUnloading) {
		...
		if(!connectedParts.remove(part)) {
			BeefCoreLog.warning("[%s] Double-removing part (%d) @ %d, %d, %d, this is unexpected and may cause problems. If you encounter anomalies, please tear down the reactor and rebuild it.", worldObj.isRemote?"CLIENT":"SERVER", part.hashCode(), part.getPos().getX(), part.getPos().getY(), part.getPos().getZ());
		}
	...
	private void _onAssimilated(MultiblockControllerBase otherController) {
		if(referenceCoord != null) {
			if(worldObj.getChunkProvider().getLoadedChunk(referenceCoord.getChunkX(), referenceCoord.getChunkZ()).isLoaded()) {
				TileEntity te = this.worldObj.getTileEntity(new BlockPos(referenceCoord.x, referenceCoord.y, referenceCoord.z));
	...
	public final void updateMultiblockEntity() {
		...
		if(worldObj.isRemote) {
			...
			if(minimumCoord != null && maximumCoord != null &&
					 this.worldObj.isAreaLoaded(new BlockPos(minimumCoord.x, minimumCoord.y, minimumCoord.z),
							 						new BlockPos(maximumCoord.x, maximumCoord.y, maximumCoord.z))) {
	...
	private void auditParts() {
		...
		for(IMultiblockPart part : connectedParts) {
			if(part.isInvalid() || worldObj.getTileEntity(new BlockPos(part.getPos().getX(), part.getPos().getY(), part.getPos().getZ())) != part) {
				onDetachBlock(part);
				deadParts.add(part);
			}
		}
	...
	public Set<IMultiblockPart> checkForDisconnections() {
		...
		for(IMultiblockPart part : connectedParts) {
			...
			if(worldObj.getTileEntity(new BlockPos(part.getPos().getX(), part.getPos().getY(), part.getPos().getZ())) != part) {
				deadParts.add(part);
				onDetachBlock(part);
				continue;
			}
	...
	protected void markReferenceCoordForUpdate() {
		...
		if(worldObj != null && rc != null) {
			worldObj.notifyBlockUpdate(new BlockPos(rc.x, rc.y,rc.z), worldObj.getBlockState(new BlockPos(rc.x, rc.y,rc.z)), worldObj.getBlockState(new BlockPos(rc.x, rc.y,rc.z)), 0);
		}
	}
	...
	protected void markReferenceCoordDirty() {
		...
		worldObj.markChunkDirty(new BlockPos(referenceCoord.x, referenceCoord.y, referenceCoord.z), saveTe);
	}

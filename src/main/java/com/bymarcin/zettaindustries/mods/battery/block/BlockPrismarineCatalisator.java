...
public class BlockPrismarineCatalisator extends BasicBlock {
    private Block fluid;
    private List<String> info = new ArrayList<String>();

    public BlockPrismarineCatalisator(Block fluid) {
        super(Material.ROCK, "blockprismarinecatalisator");
        ...
        if (world.getBlockState(pos.up()).getBlock() == Blocks.FIRE) {
            for (int i = pos.getX() - 1; i <= pos.getX() + 1; i++) {
                for (int j = pos.getZ() - 1; j <= pos.getZ() + 1; j++) {
                    ...
                    if (FluidRegistry.getFluid("sulfuricacid").equals(f)) {

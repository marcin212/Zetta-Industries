...
public class GuiControler extends GuiContainer{
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		fontRendererObj.drawString("Stored Energy: " + MathUtils.addSI(battery.getStorage().getRealEnergyStored() , "RF") + "("+(int)(percent*100D+0.5) +"%)", 0, 0, 0x000);
		fontRendererObj.drawString("Max Stored Energy: " + MathUtils.addSI(battery.getStorage().getRealMaxEnergyStored() , "RF"), 0, 10, 0x000);
		fontRendererObj.drawString("Max (In/Out)/Electrode: " + MathUtils.addSI(battery.getStorage().getMaxReceive() , "RF"), 0, 20, 0x000);
		GL11.glPopMatrix();
	}
}

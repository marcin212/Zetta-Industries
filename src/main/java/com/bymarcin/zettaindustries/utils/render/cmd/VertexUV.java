...
public class VertexUV extends RenderCommand {
	int corner;
	public VertexUV(float x, float y, float z, float u, float v, int corner) {
		super(VERTEXUV);
	...
	public IRenderCommandExecutor getExecutor(LinkedList<Matrix4f> transformations, float minU, float maxU, float minV, float maxV) {
		...
		return new VertexUVExecutor(n.x/n.w , n.y/n.w, n.z/n.w, RenderUtils.lerp(minU, maxU, args[3]), RenderUtils.lerp(minV, maxV, args[4]), corner);
	}
}
